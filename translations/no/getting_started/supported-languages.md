...
Tabellen nedenfor viser språk som for øyeblikket støttes av **Co-op Translator**. Den inkluderer språkkoder, språknavn og eventuelle kjente problemer knyttet til hvert språk. Hvis du ønsker å legge til støtte for et nytt språk, vennligst legg til tilsvarende språkkode, navn og passende skrifttype i filen `font_language_mappings.yml` som ligger i `src/co_op_translator/fonts/` og send inn en pull request etter testing.
