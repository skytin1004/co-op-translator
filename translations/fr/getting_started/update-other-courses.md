...
- S’applique à : uniquement aux dépôts Microsoft Beginners
...
> [!NOTE]
> Si vous avez une liste existante, il suffit de la mettre entre les mêmes marqueurs. La prochaine exécution la remplacera par le contenu standardisé le plus récent.
...
4. La prochaine fois que Co‑op Translator s’exécutera (CLI ou GitHub Action) dans un dépôt ciblé, il synchronisera automatiquement la section mise à jour

Cela garantit une source unique de vérité pour le contenu « Autres cours » dans tous les dépôts Beginners.

## Taille des dépôts

Les dépôts peuvent devenir volumineux en raison du nombre de langues traduites pour aider les utilisateurs finaux à fournir des instructions sur la façon d’utiliser clone - sparse pour ne cloner que les langues nécessaires et pas l’intégralité du dépôt
