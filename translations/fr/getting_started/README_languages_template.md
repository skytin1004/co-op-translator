...
- Si vous souhaitez que Co-op Translator maintienne cette liste entièrement à jour automatiquement lorsque vous exécutez `translate` (quel que soit le choix de langue), conservez les deux marqueurs de commentaire tels quels.
- Si vous souhaitez uniquement afficher un sous-ensemble de langues, supprimez les deux marqueurs de commentaire et retirez les langues que vous ne voulez pas lister. Après avoir supprimé les marqueurs, Co-op Translator ne remplacera plus automatiquement cette section.

- La section inclut désormais un avertissement "Préférez cloner localement ?" pour aider les utilisateurs à cloner sans le gros payload des traductions. Vous pouvez personnaliser l’avertissement avec l’URL de votre dépôt en exécutant, par exemple :
