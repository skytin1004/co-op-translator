...
- 如果您希望 Co-op Translator 在執行 `translate`（任何語言選取）時，自動保持此清單的最新狀態，請保持這兩個註解標記不變。
- 如果您只想顯示部分語言，刪除這兩個註解標記並移除任何不想列出的語言。移除標記後，Co-op Translator 將不再自動取代此區段。

- 本區段現包含「偏好本地複製？」的建議，以協助用戶在不載入大型翻譯資料的情況下複製。您可以透過執行下列指令並加入您的倉庫 URL 來個人化此建議，例如：
...
**免責聲明**：  
本文件係使用 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 所翻譯。雖然我們致力於確保準確性，但請注意，自動翻譯可能包含錯誤或不準確之處。原始語言文件應被視為正式權威來源。對於重要資訊，建議採用專業人工翻譯。我們不對因使用本翻譯而引起之任何誤解或曲解承擔任何責任。
