...
- 如果您希望 Co-op Translator 在執行 `translate`（任何語言選擇）時，自動保持此列表完全更新，請保持這兩個註解標記不變。  
- 如果您只想顯示部分語言，請刪除這兩個註解標記，並移除您不想列出的語言。移除標記後，Co-op Translator 將不再自動替換此區段。

- 此區段現在包含「是否Prefer to Clone Locally?」的建議，協助使用者在無需大宗翻譯檔案的情況下進行克隆。您可以透過執行類似以下指令，使用您的儲存庫 URL 個人化此建議，例如：  
...
**免責聲明**：  
本文件由 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。雖然我們致力於確保準確性，但請注意自動翻譯可能包含錯誤或不準確之處。原版文件應視為權威來源。對於重要資訊，建議採用專業人工翻譯。我們不對因使用本翻譯而產生的任何誤解或誤釋承擔責任。
