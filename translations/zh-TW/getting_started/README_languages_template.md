...
- 如果您希望 Co-op Translator 在執行 `translate`（任意語言選擇）時自動保持此清單的最新狀態，請保持這兩個註解標記不變。
- 如果您只想顯示部分語言，請刪除這兩個註解標記，並移除不想列出的語言。移除標記後，Co-op Translator 將不再自動置換此區段。

- 此區段現在包含「偏好本機複製？」的建議，協助使用者避免因大型翻譯內容下載而延遲複製。您可以透過執行如下指令來個人化建議，並指定您的儲存庫 URL，例如：
...
**免責聲明**：  
本文件係使用 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。雖然我們致力於確保準確性，但請注意自動翻譯可能包含錯誤或不準確之處。原始語言版本之文件應視為權威資料來源。對於重要資訊，建議採用專業人工翻譯。我們不對因使用本翻譯而產生的任何誤解或誤譯負責。
